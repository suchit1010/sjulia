# ===============================================
# 🚀 MULTI-AI ARBITRAGE ANALYZER
# ===============================================
# Uses Gemini, XAI, or OpenAI as fallbacks for Groq
...
function Get-AIAnalysis($priceData, $opportunities) {
    ...
    if ($profitableOpps) {
        ...
        $analysis += "ℹ️  No highly profitable opportunities detected (all spreads <0.5%)`n"
    }
    
    $analysis += "`n📊 MARKET CONDITIONS:`n"
...
function Calculate-Opportunities($priceData) {
    $opportunities = @()
...
# Main execution
Write-Host "`n🔍 Fetching cryptocurrency prices..." -ForegroundColor Cyan
$priceData = Get-CryptoPrices

Write-Host "`n📊 Calculating arbitrage opportunities..." -ForegroundColor Cyan
$opportunities = Calculate-Opportunities $priceData
...
Write-Host "`n🤖 Getting AI analysis..." -ForegroundColor Blue
$aiAnalysis = Get-AIAnalysis $priceData $opportunities
Write-Host "`n$aiAnalysis" -ForegroundColor White

Write-Host "`n🔑 API SETUP RECOMMENDATIONS:" -ForegroundColor Yellow
Write-Host "=============================" -ForegroundColor Yellow
Write-Host "To get AI-powered analysis, add one of these API keys to your .env:" -ForegroundColor White
Write-Host "• OpenAI API: https://platform.openai.com/api-keys" -ForegroundColor Cyan
Write-Host "• XAI Grok API: https://console.x.ai/" -ForegroundColor Cyan  
Write-Host "• Anthropic Claude API: https://console.anthropic.com/" -ForegroundColor Cyan
Write-Host "`n✅ SCAN COMPLETE! Current system working with rule-based analysis." -ForegroundColor Green
